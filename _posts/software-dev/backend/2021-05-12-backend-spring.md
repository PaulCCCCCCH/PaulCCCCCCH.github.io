...
但是，这样仍然不够完美。我们并没有考虑到 `Error` 的可能性（比如查询的用户不存在）。并且，我们回复的消息过于 `干货` 了：除了数据内容之外，啥也没有。这对前端来说非常不友好。我们建议大家使用 `CommonResult`（[点击这里下载它的代码](/assets/code/CommonResult.zip)） 对返回值进行封装，这样任何的返回消息，都会是以下格式：  
```json
{
